-- select 기본
-- ex) employees 테이블에서 직원의 이름, 성, 성별, 입사일 출력
...
select concat(first_name, '  ', last_name) as '전체 이름', gender as 성별, hire_date as '입사 날짜' from employees a;
...
-- order by
-- ex) employees 테이블에서 직원의 전체이름,  성별, 입사일을  입사일 순으로 출력
...
select * from salaries where from_date like '2001%' order by salary desc;
...
-- ex3) dept_emp 테이블에서 부서 번호가 d005나 d009에 속한 사원의사번, 부서번호 출력
